...
enum __attribute__((__packed__)) lux_command {
    ...
    // LUX_CMD_FRAME_FLIP[_ACK]: no index, no request payload, no/ack+crc response
    // - Request: Empty
    // - Response: None for LUX_CMD_FRAME_FLIP; ack+crc for LUX_CMD_FRAME_FLIP_ACK
    // Flip buffer so frame previously loaded with LUX_CMD_FRAME_HOLD is output.
    LUX_CMD_FRAME_FLIP = 0x90,      //TODO
    LUX_CMD_FRAME_FLIP_ACK = 0x91,  //TODO
    
    // LUX_CMD_FRAME[_HOLD][_ACK]: index, varlen request payload, no/ack+crc response
    // - Request: LED strip frame data
    // - Response: None for LUX_CMD_FRAME[_HOLD]; ack+crc for LUX_CMD_FRAME[_HOLD]_ACK
    // LUX_CMD_FRAME: Immediately output; LUX_CMD_FRAME_HOLD: store until LUX_CMD_FRAME_FLIP command
    LUX_CMD_FRAME = 0x92,
